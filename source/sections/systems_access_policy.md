...
* Requests for access to HealthTensor Platform systems and applications is made formally to the VP of Engineering, Privacy Officer, or Security Officer.
* Access is not granted until receipt, review, and approval by the HealthTensor Security Officer;
* The request for access is retained for future reference.
* All access to HealthTensor systems and services are reviewed and updated on an bi-annual basis to assure proper authorizations are in place commensurate with job functions. The form used to conduct account review is [here](https://docs.google.com/a/catalyze.io/forms/d/1oeejMta4XLiUsdH2gTKQ-glec6DBiwPJoY9F60HmPWk/viewform).
* Any HealthTensor workforce member can request change of access using this [form](https://docs.google.com/a/catalyze.io/forms/d/1ySICzCyEEdNqxHHErjlJqREBijwxs9z72L-rWXrxkm0/viewform).
...
* In the case of non-personal information, such as generic educational content, identification and authentication may not be required. This is the responsibility of HealthTensor Customers to define, and not HealthTensor.
...
* Generic accounts are not allowed on HealthTensor systems.
...
* HealthTensor maintains a minimum necessary approach to access to Customer data. As such, HealthTensor, including all workforce members, does not readily have access to any ePHI.

## Access Authorization

* Role based access categories for each HealthTensor system and application are pre-approved by the Security Officer or VP of Engineering.
* HealthTensor utilizes hardware and software firewalls to segment data, prevent unauthorized access, and monitor traffic for denial of service attacks.
...
* Access to the HealthTensor Platform systems and applications is controlled by requiring unique User Login ID’s and passwords for each individual user and developer.
...
* Shared accounts are not allowed within HealthTensor systems or networks.
...
* Workstation hard drives are encrypted using FileVault 2.0 or equivalent
* All workstations have firewalls enabled to prevent unauthorized access unless explicitly granted.

## Wireless Access Use

* HealthTensor production systems are not accessible directly over wireless channels.
...
* User IDs and passwords are used to control access to HealthTensor systems and may not be disclosed to anyone for any reason.
...
* On all production systems and application in the HealthTensor environment, password configurations are set to require that passwords are a minimum of 8 character length, 90 day password expiration, account lockout after 5 invalid attempts, password history of last 4 passwords remembered, and account lockout after 15 minutes of inactivity.
